...
(function (document, window) {
    ...
    function onMetaClick(elmt, name, navArea, locId, contentId, openBehavior, slotN) {
        providerName = name;
    ...
    function trackMetaBounce() {
        ...
        if (focusTimestamp > metaClickTimestamp && focusTimestamp - metaClickTimestamp <= TEN_MINUTES) {

            ta.trackEventOnPage('COMMERCE_CLICK', 'Meta_Bounce', (window.pageServlet || "") + '|' + (slot || "") + '|' + (providerName || ""), (focusTimestamp - metaClickTimestamp));

            metaClickTimestamp = false;
            blurTimestamp = false;
            ta.fireEvent("metaBounce");
        }
    }
