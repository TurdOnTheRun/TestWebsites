window.cobrowse=(function($){var CMD_CURRENT_WINDOW="CW";
...
i++){result+=".cbHilightElement"+SUPPORTED_HIGHLIGHT_COLORS[i]+" input, .cbHilightElement"+SUPPORTED_HIGHLIGHT_COLORS[i]+" button, .cbHilightElement"+SUPPORTED_HIGHLIGHT_COLORS[i]+" textarea {background-color: #"+SUPPORTED_HIGHLIGHT_COLORS[i]+" !important;}";
...
for(var i=0;
i<SUPPORTED_HIGHLIGHT_COLORS.length;
i++){result+=".cbHilightElement"+SUPPORTED_HIGHLIGHT_COLORS[i]+" input, .cbHilightElement"+SUPPORTED_HIGHLIGHT_COLORS[i]+" button, .cbHilightElement"+SUPPORTED_HIGHLIGHT_COLORS[i]+" select, .cbHilightElement"+SUPPORTED_HIGHLIGHT_COLORS[i]+" textarea {background-color: #"+SUPPORTED_HIGHLIGHT_COLORS[i]+" !important;}";
...
}}function removeCobEndButton(){var cobMgr=getCoBrowseMgr();
if(cobMgr!=null){if(cobrowse.isPersistentWindow&&document.getElementById(cobMgr.cobEndButtonID)!=null){document.getElementById(cobMgr.cobEndButtonID).parentNode.remove();
...
}function getEmbeddedImageUsingXMLHTTPRequest(url){var imageUrl=url;
...
}catch(e){logError("Error(sendEmbeddedImageUsingXMLHTTPRequest) while calling xhr.onreadystatechange: with"+imageUrl,e);
putEmbeddedResource(imageUrl,"")
}}else{log("Cannot load image ["+imageUrl+"]");
putEmbeddedResource(imageUrl,"")
}this.onreadystatechange=null;
xhr=null
}};
xhr.send(null)
}function getEmbeddedImageUsingCanvas(url){var imageUrl=url;
var image=new Image();
...
putEmbeddedResource(imageUrl,base64Code)
}catch(e){logError("Error(sendEmbeddedImageUsingCanvas) in image.onload: with"+imageUrl,e)("onload in sendEmbeddedImageUsingCanvas: "+imageUrl);
putEmbeddedResource(imageUrl,"")
}};
image.onerror=function(){log("Cannot load image ["+imageUrl+"]");
putEmbeddedResource(imageUrl,"")
};
image.src=url
...
}function getEmbeddedCssUsingStyleSheets(url){for(var i=0;
i<parent.document.styleSheets.length;
i++){var styleSheet=parent.document.styleSheets[i];
if(styleSheet.href&&_convertToAbsoluteUrl(styleSheet.href)==url&&isValidMediaType(styleSheet)){var data=getStyleSheetContent(styleSheet);
...
function deleteUpDireSection(url){var path=url;
while(path.match(URL_REGEXP_DIRUP)){path=path.replace(URL_REGEXP_DIRUP,"/")
}return path
...
var STATE_CHECKED=1;
var STATE_SENT=2;
function addEmbeddedResource(absoluteUrl){var correctedUrl=deleteUpDireSection(absoluteUrl);
if(!resources[correctedUrl]){resources[correctedUrl]={state:STATE_NEW,correctedUrl:getEmbeddedResourceURL(correctedUrl),hash:null,data:null};
...
},background:function(obj,name,value){inlineAttr(name,_convertToAbsoluteUrl(value))
}};
...
function isBlockTag(tagName){return blockRegex.test(tagName)
...
}function getFakeTag(blockElement){return{tag:blockElement?"div":"span",processTagBody:drop,attrProcessor:drop}
}function generateStyle(obj,backgroundImage,tag){var styleStr="";
if(tag=="span"){styleStr+="display:inline-block;"
...
EMBED_TAG_FILTER.attributes={style:function(node){return generateStyle(node,getServerPath()+"flash_logo.png",EMBED_TAG_FILTER.tag)
}};
var HIDDEN_BLOCK_TAG_FILTER=getFakeTag(true);
HIDDEN_BLOCK_TAG_FILTER.attributes={style:function(node){return generateStyle(node,null,HIDDEN_BLOCK_TAG_FILTER.tag)
}};
var HIDDEN_INLINE_TAG_FILTER=getFakeTag(false);
HIDDEN_INLINE_TAG_FILTER.attributes={style:function(node){return generateStyle(node,null,HIDDEN_INLINE_TAG_FILTER.tag)
}};
...
}catch(e){logError("Error(CANVAS_TAG_FILTER.attributes = { style: function(node) })",e);
return generateStyle(node,null,CANVAS_TAG_FILTER.tag)
}}};
var TAG_FILTERS={div:{filter:function(obj){return obj.id!=DIV_ID_COBROWSE_BANNER
...
},style:function(obj){if(obj.id=="inqChatStage"&&Inq.CHM.isPersistentChatActive()){return"display: none !important; visibility: hidden;"
...
}}},textarea:{processTagBody:function(obj){data.push(htmlFilter.content($(obj),obj.value))
}},style:drop,script:drop,noscript:drop};
var data=[];
function inlineAttr(name,value){data.push(" ",name,'="',(value.replace?value.replace(/"/gi,'\\"').replace(/&/gi,"&amp;").replace(/>/gi,"&gt;"):value),'"')
...
var tagfilter;
if(htmlFilter.isHiddenNode&&htmlFilter.isHiddenNode($(this))){if(isBlockTag(tag)){tagfilter=HIDDEN_BLOCK_TAG_FILTER
}else{tagfilter=HIDDEN_INLINE_TAG_FILTER
...
if(tagfilter.processTagBody){tagfilter.processTagBody(this)
...
}function log(text,send2server){if(window.Inq!=null&&window.Inq.log!=null){Inq.log("CBC: "+text)
}if(send2server===true){sendByJsonP(getJsonpUrl(),text,"INFO")
}}function logInfo(message){log(message,true)
}function logError(message,e){var errorDetails=message;
if(logError.caller!=null){errorDetails+=",at:"+logError.caller.toString().split("{")[0]
}if(e&&e.message){errorDetails+=",\nDetail:"+e.message;
if(e.stack!=null){errorDetails+=",\nStack Trace:"+e.stack
}}log(errorDetails);
sendByJsonP(getJsonpUrl(),errorDetails,"ERROR");
return function(value){log("   Trace value:["+value+"]")
}
}function sendByJsonP(url,data,level){var jpMsg="cob.clientError?cid="+getChatID()+"&wid="+cobrowse.windowId+"&message="+data;
var d=new Date();
...
if(head){var links=div.getElementsByTagName("LINK");
for(var i=0;
i<links.length;
...
}}}}function supportSuppressedLabel(){var agentAssisted=parent.document.getElementById("agentAssisted");
var agentAssistedSuppressed=parent.document.getElementById("agentAssistedSuppressed");
if((agentAssisted)&&(agentAssistedSuppressed)){if(isSuppressed()){agentAssisted.style.display="none";
agentAssistedSuppressed.style.display="block"
}else{agentAssisted.style.display="block";
...
}},hide:function(){toggle(divs.banner,false);
this.termsAndConditions(false)
},show:function(){if(divs){toggle(divs.banner,true)
}else{divs=createBannerDiv();
toggle(divs.banner,true)
}supportSuppressedLabel()
},setCobrowseBannerText:function(bannerText){if(divs&&(divs.banner)&&(bannerText)){divs.banner.innerHTML=bannerText
}}}
}();
...
},cmd:"TA",getCmdData:function(ix,value,input){return{IX:ix,TEXT:htmlFilter.content($(input),value)}
...
}}}function requestAICommand(){var url=getServerPath()+"cbs/command.js?"+ajaxParams()+"&CSQ="+client_csq;
...
}}if(data.length>0){sendToCBS(CMD_CHECK_EMBEDDED_RESOURCES,data.join(";"),"action="+CMD_CHECK_EMBEDDED_RESOURCES)
}}function sendToCBS(command,data,params,boxID){if(Inq&&Inq.ROM){var url=getServerPath()+"cbs/cobrowse?SEQ=n&APG="+isAcceptedPage+"&"+ajaxParams()+((params)?"&"+params:"");
boxID=(typeof boxID!="undefined")?boxID:getBoxId("cbcPost");
try{var queueItem={command:command,request:["COBROWSE",boxID,Inq.getSiteID(),getInqFrameUrl(),url,data],boxID:boxID};
postQueue.push(queueItem);
postQueueItem(false)
}catch(e){log("Error: "+e.message)
}}}var uniqBoxNum=1;
function getBoxId(prefix){return((prefix)?""+prefix:"")+uniqBoxNum++
}var waitingAnswer=false;
var lastCommandTime=0;
function postQueueItem(flag){if(waitingAnswer&&flag){waitingAnswer=false
...
}function sendCommand(command,data,boxID){var dataStrings=[];
...
return data
}function addHostedLocation(data){data.LOC=parent.location.href;
return data
...
}function watchMouse(ev){var e=ev?ev:event;
...
}else{if(typeof parent.document.documentElement.scrollTop!="undefined"&&parent.document.documentElement.scrollTop>0){position=[parent.document.documentElement.scrollLeft,parent.document.documentElement.scrollTop]
...
}function getChatID(){if(window.Inq!=null&&(Inq.CHM)){return Inq.CHM.getChatID()
}else{log("chat manager [Inq.CHM] is not initialized")
}return 0
}function isValidChatID(){return(getChatID()!=0)
}function whenActiveWindow(){if(isValidChatID()){clearTimer();
if(activeWindow||windowHasFocus()){if(isSuppressed()){sendCommand(CMD_SUPPRESS,{},CMD_SUPPRESS)
}else{sendCommand(CMD_CURRENT_WINDOW,{},CMD_CURRENT_WINDOW)
}}setTimerInterval()
}}function getHTML(){try{lastBodySentArray=[];
if(isValidChatID()){if(isSuppressed()){cobrowseSuppressedPage()
}else{sendHead();
sendBody()
}}return true
...
}function clearTimer(){try{if(pageUpdateTimer!=null){window.clearTimeout(pageUpdateTimer)
}pageUpdateTimer=null
}catch(e){logError("Error(clearTimer)",e)
}}function setTimerInterval(){if(pageUpdateTimer==null){pageUpdateTimer=window.setTimeout(checkForUpdates,TIMERINTERVAL_PAGE_UPDATE)
}}function setAuthMode(mode){if(mode.auth){var wasNotAuthorized=!authorized;
authorized=mode;
if(cobrowse.isPersistentWindow){clearTimer()
}else{if(wasNotAuthorized){whenActiveWindow();
getHTML();
banner.show();
var cobMgr=getCoBrowseMgr();
if(cobMgr!=null){if(!(cobrowse.isPersistentWindow&&cobrowse.isIE)){try{cobMgr.focusCobEndBtn()
}catch(errDom){}}}clearTimer();
setTimerInterval()
...
function _convertToAbsoluteUrl(url){try{return convertToAbsoluteUrl(url)
}catch(e){logError("Error(_convertToAbsoluteUrl): ",e);
return url
}}function convertToAbsoluteUrl(url){var parentLocation=parent.window.location;
var parentDomain=getDomain(parentLocation);
var parentPath=parentLocation.pathname;
if(url==null||url.length==0){return parentDomain+parentPath
}else{url=url.trim();
if(url.match(URL_REGEXP_IS_ABSOLUTE)){if(url.charAt(0)=="/"&&url.charAt(1)=="/"){return parentLocation.protocol+url
}else{return url
}}else{var firstChar=url.charAt(0);
if(firstChar=="/"){return parentDomain+url
}else{if(firstChar=="#"||firstChar=="?"||firstChar==";"){return parentDomain+parentPath+url
}else{return parentDomain+parentPath.match(URL_REGEXP_GRAB_TO_RIGHTMOST_SLASH)+url
}}}}}var URL_REGEXP_SIMPLE=new RegExp("url","i");
var URL_REGEXP_REPLACE=/(^.*)(url)(\()([^(]*)(\).*)($)/gi;
function fixLinkDataInStyle(data){if(data.match(URL_REGEXP_SIMPLE)){var line;
...
if(isValidMediaType(styleSheet)){if(styleSheet.href){var url=_convertToAbsoluteUrl(styleSheet.href);
if(URL_REGEXP_CHECK_HTTP_PROTOCOL.test(url)){if(isEmbeddedResource(url)){cssLink+="<link type='text/css' rel='stylesheet' href='"+getFullEmbeddedResourceURL(url)+"' />\n";
addEmbeddedResource(url);
getStyleSheetContent(styleSheet)
}else{cssLink+="<link type='text/css' rel='stylesheet' href='"+url+"' />\n"
}}else{log("StyleSheet index "+i+" has incorrect URL and can't be used, HREF: "+url)
}}else{cssText+=getStyleSheetContent(styleSheet)
}}}catch(e){logError("Error(getCss/block error of unaccessible property):",e)
}}cssText="\n<style type='text/css'>\n<!--\n"+cssText+cssHighlightStyleAI+"\n-->\n</style>\n"
}catch(e){logError("Error(getCss):",e)
...
if(!isTest){sendCommand(CMD_HEAD,data,CMD_HEAD)
}}catch(e){logError("Error(sendHead): head sending"+(isTest?" while performance testing":""),e)
}}function getStyleSheetContent(styleSheet){var cssContent="";
var rules=styleSheet.cssRules||styleSheet.rules;
if(rules!=null){for(var j=0;
j<rules.length;
j++){try{if(rules[j].type!=7){if(rules[j].cssText){cssContent+=fixLinkDataInStyle(rules[j].cssText)
}}}catch(e){logError("Error(getStyleSheetContent): undefined or something else",e)
}}}return cssContent
...
var data={};
var i=0;
for(var bodyRest=bodyHtmlArray.join("");
bodyRest.length>0;
...
if(!isTest){sendCommand(CMD_BODYPART,data,CMD_BODYPART+i++)
...
}function sendUpdate(){var newBodyArray=$(parent.document.body).generateHtml();
...
function cobrowseSuppressedPage(){var cobrowseManager=getCoBrowseMgr();
var result=false;
if(cobrowseManager){try{result=cobrowseManager.cobrowseSuppressedPage()
}catch(e){logError("Error (cobrowseSuppressedPage)",e)
}}if(!cobrowseManager||!result){sendCobrowseMessage(KEY_COBROWSE_EVENT_TYPE_CLIENT_COBROWSE_SUPPRESSED,Inq.getLocalizedMessage("cobrowseSuppressed"))
}}function cbcFailHandler(cbcFailMessage){var cobrowseManager=getCoBrowseMgr();
var result=false;
if(cobrowseManager){try{result=cobrowseManager.cobrowseFailHandler(cbcFailMessage)
}catch(e){logError("Error (cbcFailHandler)"+cbcFailMessage,e)
}}if(!cobrowseManager||!result){sendCobrowseMessage(KEY_COBROWSE_EVENT_TYPE_CLIENT_COBROWSE_END,cbcFailMessage)
}}function performanceTest(silent){var performance=null;
try{performance=calculateCobrowsePerformance();
var cobrowseManager=getCoBrowseMgr();
if(cobrowseManager){try{cobrowseManager.performanceTest(performance)
}catch(err){logError("Error (performanceTest: send results)",err)
}}else{if(!silent){sendCobrowseMessage(KEY_COBROWSE_EVENT_TYPE_CLIENT_PERFORMANCE_TEST,Inq.getLocalizedMessage("customerPerformanceTest")+execTime+"ms")
}}}catch(e){logError("Error (performanceTest: calculateCobrowsePerformance)",e);
if(!silent){cbcFailHandler(Inq.getLocalizedMessage("cobrowseInitializationFail"))
}}return performance
}function getCoBrowseMgr(){try{if((Inq.CM)&&(Inq.CM.persistentWindow)&&(Inq.CM.persistentWindow.inqFrame)&&(Inq.CM.persistentWindow.inqFrame.com)){return Inq.CM.persistentWindow.inqFrame.com.inq.flash.client.chatskins.CoBrowseMgr
}if((inqFrame.com)&&(inqFrame.com.inq)){return inqFrame.com.inq.flash.client.chatskins.CoBrowseMgr
}}catch(e){logError("Error (getCoBrowseMgr)",e)
}return null
}var ciProxy=function(){return{agentEndsCob:function(){try{getCoBrowseMgr().agentEndsCob()
}catch(e){logError("Error(ciProxy.agentEndsCob)",e)
}},sendCobrowseEnded:function(){var cobrowseManager=getCoBrowseMgr();
if(cobrowseManager){try{cobrowseManager.sendCobrowseEnded()
}catch(e){logError("Error(ciProxy.sendCobrowseEnded)",e)
}}else{sendCobrowseMessage(KEY_COBROWSE_EVENT_TYPE_CLIENT_COBROWSE_END,Inq.getLocalizedMessage("customerEndCobrowseSession"))
}}}
}();
function sendCobrowseMessage(cobrowseEvent,messageText){try{Inq.ROM.sendCobrowseMessage(Inq.CHM.getChatID(),Inq.CHM.getAgentID(),Inq.getCustID(),cobrowseEvent,messageText)
}catch(e){logError("Error (sendCobrowseMessage): cobrowseEvent = "+cobrowseEvent+" message = "+messageText,e)
}}function authorize(auth){var authObj={auth:auth};
var authJson=Inq.CM.JSON.stringify(authObj);
var expiry=(auth)?"":(new Date((new Date()).getTime()+COOKIE_EXPIRATION*1000)).toGMTString();
...
}function terminate(callback){resources={};
banner.hide();
if(authorized){authorize(cobrowse.NOT_SHARED);
if(callback){callback()
}authorized=null
}return false
}function terminateFromPersistentWindow(callback){resources={};
...
if((rule.pageMarker&&rule.pageMarker==getPageMarker())||(rule.urlRegex&&window.parent.document.URL.match(rule.urlRegex))||(rule.pageId&&Inq.LDM.pageCheck(rule.pageId))||(rule.markerRegex&&getPageMarker().match(rule.markerRegex))||(rule.contentGroupId&&Inq.LDM.checkCG(rule.contentGroupId))){result=true
}return result
...
}}function checkForUpdates(){try{clearTimer();
if(cobrowse.isPersistentWindow){return
}if(authorized){if(!lastBodySentArray.isEmpty()&&!isSuppressed()){forAllInputTypes(checkForInputChanges);
if(updateScrollChanges()){sendCommand(CMD_UPDATE,addScrollData({UPDATE:"",IB:0,IE:0}))
...
}function unwrap(obj){var parent=obj.parentNode;
if($(parent).hasClass(CSS_CLASS_HIGHLIGHT_TEXT)){$(obj).unwrap();
return
}for(var i=0;
i<SUPPORTED_HIGHLIGHT_COLORS.length;
i++){if($(parent).hasClass(CSS_CLASS_HIGHLIGHT_INPUTS+SUPPORTED_HIGHLIGHT_COLORS[i])){$(obj).unwrap();
return
}if($(parent).hasClass(CSS_CLASS_HIGHLIGHT_SELECT+SUPPORTED_HIGHLIGHT_COLORS[i])){$(obj).unwrap();
return
}}}function wrapText(clazz,color,text){return"<span class='"+clazz+"'"+(color?" style='display: inline !important; background-color: "+color+"'>":">")+text+"</span>"
}function wrapInputs(clazz,text){return"<span class='"+clazz+"' style='display: inline-block !important;'>"+text+"</span>"
...
}function highlightElement(color){try{if(!color){throw new Error(" color parameter is invalid for ["+color+"]")
}var parent=this.parentNode;
var nodeName=this.nodeName;
if(parent==null){throw new Error(" Parent is null for "+nodeName)
}else{if(nodeName=="#text"&&/[^ \t\n\r]/.test(this.textContent)){if($(parent).hasClass(CSS_CLASS_HIGHLIGHT_TEXT)&&colorToHex(parent.style.backgroundColor)==color.toLowerCase()){log("Node already highlighted by specified color ["+color+"]")
}else{if($(parent).hasClass(CSS_CLASS_HIGHLIGHT_TEXT)&&parent.childNodes.length==1){unwrap(this)
}$(this).wrap(wrapText(CSS_CLASS_HIGHLIGHT_TEXT,color,""))
}}else{if(nodeName=="SELECT"){unwrap(this);
$(this).wrap(wrapInputs(CSS_CLASS_HIGHLIGHT_SELECT+color.replace("#",""),""))
}else{if(nodeName=="BUTTON"||nodeName=="INPUT"||nodeName=="TEXTAREA"){unwrap(this);
$(this).wrap(wrapInputs(CSS_CLASS_HIGHLIGHT_INPUTS+color.replace("#",""),""))
}else{if(nodeName!="#comment"&&nodeName!="IFRAME"&&nodeName!="STYLE"&&nodeName!="SCRIPT"&&nodeName!="NOSCRIPT"){$(this).contents().each(highlightElement,[color])
}}}}}}catch(e){logError("Error (highlightElement)",e)
}}function stripEnd(node,offset){try{if(node.nodeValue==null){return
}if(offset!=-1){var text=node.nodeValue;
...
}}function stripStart(node,offset){try{if(node.nodeValue==null){return
...
}}function traverse(e1,e2,fn){var commonParent=$(e1).closest($(e2).parents()).get(0);
var current=e1;
while(current.parentNode!=commonParent){if(current.nextSibling){current=current.nextSibling;
fn(current)
}else{current=current.parentNode
}}var p2=$(e2).parentsUntil(commonParent).get();
p2.unshift(e2);
var to=p2.pop();
if(current!=to){current=current.nextSibling
}while(current!=e2){if(current&&current!=to){fn(current);
current=current.nextSibling
}else{current=to.firstChild;
to=p2.pop()
}}}function highlightText(elStart,indexStart,elEnd,indexEnd,color){stripEnd(elEnd,indexEnd);
stripStart(elStart,indexStart);
var elements=[elStart,elEnd];
traverse(elStart,elEnd,function(element){elements.push(element)
});
...
if(resource){if(resource.state==STATE_CHECKED&&(resource.data)){resource.state=STATE_SENT;
...
getResource(url)
}}function getResource(url){try{if(isEmbeddedResourceCss(url)){getEmbeddedCssUsingStyleSheets(url)
}else{if(isCanvasSupported){getEmbeddedImageUsingCanvas(url)
}else{getEmbeddedImageUsingXMLHTTPRequest(url)
}}}catch(e){logError("Error(getResource) Uploading ["+url+"] failed",e)
...
}function calculateCobrowsePerformance(){var startTime=now();
...
},setCobrowseBannerText:function(bannerHtml){this.bannerHtml=bannerHtml;
if(banner){banner.setCobrowseBannerText(bannerHtml)
...
},stop:function(){if(this.isPersistentWindow){terminateFromPersistentWindow(ciProxy.sendCobrowseEnded)
}else{terminate(ciProxy.sendCobrowseEnded)
}var cobMgr=getCoBrowseMgr();
if(cobMgr!=null){cobMgr.setFocusOnChatInputField()
}},stopQuiet:terminate,reset:reset,termsAndConditions:banner.termsAndConditions,performanceTest:testGetHTML,accept:function(flagCheckPerformance){if(flagCheckPerformance){performanceTest()
}try{isAcceptedPage=true;
authorize(cobrowse.ACCEPTED)
}catch(e){logError("Error(accept)",e);
cbcFailHandler(Inq.getLocalizedMessage("cobrowseInitializationFail"));
throw e
}},acceptShare:function(flagCheckPerformance){if(flagCheckPerformance){performanceTest()
}try{isAcceptedPage=true;
authorize(cobrowse.ACCEPTED|cobrowse.SHARED_CONTROL)
}catch(e){logError("Error(acceptShare)",e);
cbcFailHandler(Inq.getLocalizedMessage("cobrowseInitializationFail"));
throw e
}},callBackAuthorized:function(json,boxID){setAuthMode(getAuthModeFromJson(json,boxID))
},callBackProxyError:function(cmd,boxID,errorStr){if(isValidChatID()){if(boxID==CMD_CURRENT_WINDOW||boxID==CMD_HEAD||boxID.indexOf(CMD_BODYPART)==0){notifyAgentUponFailure("Cobrowse data transfer failed, error code ["+boxID+"]")
}else{if(boxID==PS_CBC_AUTH){notifyAgentUponFailure("Cobrowse enable failed, error code ["+boxID+"]")
}}log(errorStr)
}},requestAICommand:function(){if(authorized){requestAICommand()
}else{cmdRequest=null
}},runCommand:function(command,csq){if(command&&csq>client_csq){client_csq=csq;
executeCommand(command)
}},runServerCommand:function(command){if(command){executeCommand(command)
}},ackReceived:function(json,boxId){try{postQueueItem(true)
}catch(e){logError("Error(ackReceived)",e)(json);
waitingAnswer=false
}},getHTML:function(){return getHTML()
},highlightInputField:function(inputId,color){$(getItemById(inputId)).each(highlightElement,[color]);
modified=true
},unhighlight:function(){$("."+CSS_CLASS_HIGHLIGHT_TEXT,window.parent.document).each(function(index){$(this).contents().unwrap()
});
for(var i=0;
i<SUPPORTED_HIGHLIGHT_COLORS.length;
i++){$("."+CSS_CLASS_HIGHLIGHT_INPUTS+SUPPORTED_HIGHLIGHT_COLORS[i],window.parent.document).each(function(index){$(this).contents().unwrap()
});
$("."+CSS_CLASS_HIGHLIGHT_SELECT+SUPPORTED_HIGHLIGHT_COLORS[i],window.parent.document).each(function(index){$(this).contents().unwrap()
})
}modified=true
...
},endCobrowse:function(){terminate(ciProxy.agentEndsCob)
...
},scrollTo:function(x,y){window.parent.scrollTo(x,y)
},sendEmbeddedResources:uploadResources,highlightText:function(startId,indexStart,endId,indexEnd,color){highlightText(getItemById(startId),indexStart,getItemById(endId),indexEnd,color);
modified=true
},initialize:function(maskingConfig,isEmbeddedResource,isIE,cobrowseBannerText,isPersistentWindow){Inq.CBC=this;
this.isIE=isIE;
this.isPersistentWindow=isPersistentWindow;
if(isPersistentWindow){htmlFilter.contentNode=createContentNode(false,function(){});
...
bindEx($(window.parent),{mousemove:watchMouse,focus:function(event){activeWindow=true;
if(!authorized){checkAuthorized()
}else{whenActiveWindow()
}},blur:function(event){activeWindow=false
}});
...
if(Inq.CHM.isChatInProgress()){checkAuthorized()
}else{authorize(cobrowse.NOT_SHARED)
}if(Inq.CHM.isPersistentChatActive()&&!Inq.CM.persistentWindow){authCheckTimer=setInterval(checkAuthorized,TIMERINTERVAL_AUTH_CHECK)
}log("Cobrowse initialization complete")
}catch(e){logError("Error(initialize)",e);
throw e
