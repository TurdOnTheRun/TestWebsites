...
function logMessageToTagServer(msg,logLevel){if(isLog4TcSupported===true){switch(logLevel){case"FATAL":tclog.fatal(msg);break;case"ERROR":tclog.error(msg);break;case"WARN":tclog.warn(msg);break;case"INFO":tclog.info(msg);break;case"DEBUG":tclog.debug(msg);break;}}else{log(msg,logLevel);if(!JSLoggingDisabled){ROM.post(urls.logJsPostURL,{level:logLevel,line:msg});}}}
...
catch(err){if(!!logOnError){logActionErr(err,rule,"JS eval error on expression "+o.toString());}
return dfltOnError;}}
function isNullOrUndefined(val){return(val==null||typeof(val)=="undefined");}
function isIE(){if(window.navigator.appName=="Microsoft Internet Explorer"||window.navigator.userAgent.indexOf("Trident")>=0){return true;}else{return false;}}
...
function isChrome(){return(window.navigator.userAgent.indexOf("Chrome")>=0||window.navigator.userAgent.indexOf("CriOS")>=0)&&(!isEdge());}
...
function getBrowserTypeAndVersion(){var nvg=window.navigator.appName;var userAgt=window.navigator.userAgent;var temp;var matches=userAgt.match(/android(?!.*chrome)/i);if(matches&&(temp=userAgt.match(/version\/([\.\d]+)/i))!=null){matches=["Android Browser",temp[1]];}
else{matches=userAgt.match(/(opera|chrome|safari|firefox|msie)\/?\s*(\.?\d+(\.\d+)*)/i);if(matches&&(temp=userAgt.match(/version\/([\.\d]+)/i))!=null){matches[2]=temp[1];}
...
function safe(id,dflt,sendErrorToTagServer){if(id){try{var o=win.eval(id);if(isNullOrUndefined(o)){return o;}
else if(typeOf(o)=="array"){return Array.clone(o);}else if(o.getTime){if(!o.equals){MI8.prepare(o).im13(DateMixIn);}}else if(o.toExponential){if(!o.inRange){MI8.prepare(o).im13(NumberMixIn);}}
return o;}catch(err){if(sendErrorToTagServer){logErrorToTagServer(err);}}}
return dflt;}
...
function secureHTTP(httpURL){return httpURL.replace(/^http:/i,"https:");}
...
return string;}
function productionFilter(hostedFileList,_productionFilter){if(_productionFilter){var filters=_productionFilter.split(",");var listItems=hostedFileList.split(",");var filteredListItems=[];var regexProduction=new RegExp(filters[0],"i");var production=regexProduction.test(inqFrame.parent.location.href);if(production){for(i=0;i<listItems.length;i++){var item=listItems[i];if(regexProduction.test(item)){filteredListItems.push(item);}}
return filteredListItems.join(",");}else if(filters.length>1){var walledGarden;var filter;for(i=1;i<filters.length;i++){filter=filters[i];walledGarden=(filter.indexOf("-")==0)?true:false;var regexSilo=new RegExp((walledGarden)?filter.substr(1):filter,"i");var silo=regexSilo.test(inqFrame.parent.location.href);if(silo){for(i=0;i<listItems.length;i++){var item=listItems[i];if(regexSilo.test(item)||(!walledGarden&&regexProduction.test(item))){filteredListItems.push(item);}}
...
if(this.set3rdPartyCookieBusy)return;this.set3rdPartyCookieBusy=true;this.set3rdPartyCookieFromQueue();};CM47.prototype.set3rdPartyCookieFromQueue=function(){var cookieQItem=null;cookieQItem=this.set3rdPartyCookieQueue.shift();if(!isNullOrUndefined(cookieQItem)){this.postRequestToIframeProxy(cookieQItem.postCookie,cookieQItem.postCookie[1]);}else{this.set3rdPartyCookieBusy=false;if(this.onThirdPartyCookieQueueEmpty!=null){this.onThirdPartyCookieQueueEmpty();}}};CM47.prototype.whenCookiesCommitted=function(handler){if(this.xd){if(this.isThirdPartyCookieQueueEmpty())
...
return null;};CM47.xdPsHelper.domains2Check.findEntryWithLatestCookie=function(){var data="";var date=null;var latestDate=null;var theobj=null;for(var i=0;i<this.length;i++){date=getDateFromData(this[i].data.data);if(date!=null){if(latestDate==null){theobj=this[i];latestDate=date;}else if(date>latestDate){theobj=this[i];latestDAte=date;}}}
...
rom.ch.remove(channel);});channel.send(url,data,null,resendOnError,maxResendTimes,timeout);this.ch.push(channel);}catch(e){logErrorToTagServer("SvrCom.sendToServer ERROR:"+e);var errMsg='RemoteOpsMgr.send >>Exception making an HTTP Request. URL '+url+catchFormatter(e);this.post(urls.loggingURL,{level:'ERROR',line:errMsg});}};this.post=function(url,data,id){try{id=id?id:"vanity"+Math.floor(Math.random()*1000001);var pstr=(typeof data=="string")?data:this.toParamString(data);var port=(inqFrame.location.port!="")?":"+inqFrame.location.port:"";var parentURL=inqFrame.location.protocol+"//"+inqFrame.location.hostname+port+inqFrame.location.pathname;var postCookieRequest=["POSTBR30",id,"",parentURL,encodeURIComponent(url+"?"+pstr)];(CM).postRequestToIframeProxy(postCookieRequest,id);}catch(e){logErrorToTagServer("SvrCom.sendToServer ERROR:"+e);}};this.postToServer=function(action,data){if(isNullOrUndefined(data)||isNullOrUndefined(action)){return;}
...
this.pageFound=(this.page!=null);};G3RE12.prototype._resolvePage=function(){var pages=this.pages;var pageURL=win.document.URL.toString();for(var i in pages){var re=pages[i].re;var caseInsensitive=false;if(re.indexOf("(?i)")===0){re=re.substr(4);caseInsensitive=true;}
var r=new RegExp(re,caseInsensitive?"i":undefined);var mat=r.exec(pageURL);if(!!mat&&mat[0]==pageURL){this.page=pages[i];break;}}};G3RE12.prototype.getCurrentPageURL=function(){try{return CHM.isPersistentWindow()?top.opener.document.URL:win.document.URL;}catch(e){return win.document.URL;}};G3RE12.prototype.getPageMarker=function(idx){var p=null;if(CHM.isPersistentWindow()){try{p=window.parent.opener.inqFrame.Inq.LDM.getPage(idx);}catch(err){logMessageToTagServer("Can't get opener page marker data for ChatID = "+CHM.getChatID()+". CustomerID = "+getCustID(),LOG_LEVELS.WARN);}}
...
return result;};function G3RE13(id,data){this._frameworkModule(id);this._observable();var rulesIndexes={};var _brMgr=this;function onDomMutation(mutations){_brMgr.onDomMutation(mutations);}
if(!!window.MutationObserver){domObserver=new MutationObserver(onDomMutation);domObserver.observe(doc.body,{childList:true,subtree:true,attributes:true});doc.body.setAttribute('data-inq-observer','1');}
if(data&&data.rules){for(var i=0;i<data.rules.length;i++){var r=data.rules[i];var id=r.getID();var overridableRuleInd=rulesIndexes[id];if(overridableRuleInd==undefined){rulesIndexes[id]=i;}else{data.rules[overridableRuleInd]=r;data.rules.splice(i,1);i--;}}}
...
return div;};C2C.prototype.clear=function(){var div=this.getDiv();if(div){div.occupied=false;div.innerHTML="";}};C2C.prototype.reset=function(){this.stopC2CAgentCheckTimer();this.clear();};C2C.prototype.show=function(setFocus){var div=this.getDiv();if(!!this.c2cSpec.abs){div.style.position="absolute";div.style.top=this.c2cSpec.abs.y;div.style.left=this.c2cSpec.abs.x;}
...
if(value==1){logMessageToTagServer(prepareLoggingContext("Chat was closed by timeout."),LOG_LEVELS.WARN);}};GR34.prototype.onPageLanding=function(evt){try{if(!!evt.reinitialized&&this.isChatInProgress())
...
this.started=true;};GR34.prototype.getOpenerChatFrame=function(){try{if(this.isPersistentWindow()&&Inq.isSameOrigin()){return Inq.parent.opener.inqFrame;}}catch(err){logMessageToTagServer("Failure to Launch: "+catchFormatter(err),LOG_LEVELS.INFO);}
return null;};GR34.prototype.persistentRefresh=null;GR34.prototype.setPersistentRefresh=function(refreshData){var ptrn="tcRefresh=";document.cookie=ptrn+refreshData+";";this.persistentRefresh=refreshData;};GR34.prototype.r23ertd=function(){var refresh=null;if(!Inq.CHM.isPersistentWindow())return null;if(Inq.CHM.persistentRefresh!=null)return Inq.CHM.persistentRefresh;var _opener=null;try{_opener=Inq.CHM.getOpenerChatFrame();}catch(e){_opener=null;}
if(_opener&&!_opener.closed&&opener.Inq.CHM["persistentRefresh"]!=null){return _opener.Inq.CHM.persistentRefresh;}
if(document.cookie.length<1)return refresh;var ptrn="tcRefresh=";var cookies=document.cookie.split(";");for(var ix=0;ix<cookies.length;ix++){var ck=cookies[ix];while(ck.substr(0,1)==" ")ck=ck.substr(1);if(ck.indexOf(ptrn)==0){ck=unescape(ck.substr(ptrn.length));refresh=Inq.CHM.persistentRefresh=(ck.length>0)?ck:null;return(refresh);}}
return refresh;};GR34.prototype.load=function(){function fixBlockedServicesData(){var blocked={};var sessionServices=sessionData["blocked"];if(!!sessionServices){for(var service in sessionServices){blocked[service]=sessionServices[service];}}
...
return null;}}catch(e){};return winTest;};GR34.prototype.againPopoutChat=function(transferURL,differentDomains){var persistentFocus=true;var winTest=null;var url,target;var port=(window.parent.document.location.port=="")?"":(":"+window.parent.document.location.port);url=(Inq.v3framesrc.indexOf("/")==0)?window.parent.document.location.protocol+"//"+window.parent.document.location.hostname+port+Inq.v3framesrc:Inq.v3framesrc;var ix=transferURL.indexOf("#");var extra="";if(ix>=0){url=transferURL.substr(0,ix);extra=transferURL.substr(ix+1);this.setPersistentRefresh(extra);}else{url=transferURL;}
...
else if(!isNullOrUndefined(this.chat)&&this.chat.pC!=status){this.chat.pC=status;}}catch(e){logMessageToTagServer("Failure on setPersistentWindowActive: "+catchFormatter(e),LOG_LEVELS.INFO);}};GR34.prototype.isPersistentWindowActive=function(){return(null!=this.r11rbbv());};GR34.prototype.isPersistentChatActive=function(){return this.isPersistentChatMaybeActive();};GR34.prototype.isPersistentChatInProgress=function(){return this.isPersistentChatActive();};GR34.prototype.isPersistentChat=function(){if(this.chat&&this.chat.pC){return true;}
...
this.setPersistentChatSetting(false);try{this._postSurveyListener();}
catch(e){}
this.setChatInterfaceData({});this.nnm89(this.getChatID());this.fireChatClosed();if(this.chat){this.chat.close();}
this.setPersistentWindowActive(false);delete this.chat;this.save();};GR34.prototype.getSurveyData=function(){return this.getChatData()!=null?this.getChatData().survey:null;};GR34.prototype.setSurveyData=function(sdat){this.getChatData().survey=sdat;};GR34.prototype.setSurveyAuxParams=function(params){if(this.getLastChat()){var svyPrms=this.getLastChat().svyPrms;if(!svyPrms){svyPrms=this.getLastChat().svyPrms=MI8.mixAbsorber({});}
svyPrms.absorb(params);this.save();}else{log("Warning: setting survey params failed - last chat data object doesn't exist.");}};GR34.prototype.getSurveyAuxParams=function(){if(this.getLastChat()){return this.getLastChat().svyPrms;}else{return null;}};GR34.prototype.setClickStreamSent=function(isSent){if(this.getLastChat()){this.getLastChat().clkstr=isSent;this.save();}else{log("Warning: setting ClickStreamSent flag failed - last chat data object doesn't exist.");}};GR34.prototype.isClickStreamSent=function(){if(this.getLastChat()){return this.getLastChat().clkstr;}else{return null;}};GR34.prototype._postSurveyListener=function(){var sdat=this.getSurveyData();if(sdat){if(this.surveyRegistered)return;this.surveyRegistered=true;}};GR34.prototype.closePersistent=function(){var chatID=this.getChatID();if(!Inq.isSameOrigin()){this.nnm89(chatID);this._postSurveyListener();}
else{try{var o=getOpener();o.inqFrame["persistentChatWindow"]=window.parent;inqFrame.Inq.MSG.closePersistent(o);}
catch(err){this.nnm89(chatID);}}};GR34.prototype.inactivateChat=function(){if(this.chat==null)return;this.chat.setVisible(false);FP.closeChatInterface();};GR34.prototype.closeChat=function(_src,_data,_reason,_closeConnection){if(this.isPersistentWindow()){this.closePersistent();}
else
{this._postSurveyListener();if(!this.persistentChat){this.fireChatClosed();}
...
return id;};GR34.prototype.getLastCallID=function(){return this.lastCallId;};GR34.prototype.getChatType=function(){return this.chat?this.chat.getChatType():null;};GR34.prototype.getLastChatType=function(){return this.lastChat?this.lastChat.chatType:null;};GR34.prototype.tryue=function(serviceType,period){if(serviceType!=GR34.CHM.CHAT_TYPES.POPUP&&serviceType!=GR34.CHM.CHAT_TYPES.POPUP_CALL){return;}
...
w.close();}catch(e){}}}
this.earlyPopout=this.popoutWindow=null;}}};GR34.prototype.reset=function(){};GR34.prototype.sendMsg=function(_msgData,_src){ROM.send(Inq.urls.sendToAgentURL,{chatID:_msgData.chatID,msg:escape(_msgData.msg)});};GR34.prototype.closeConnection=function(){};GR34.prototype.toString=function(){return"GR34: "+MI8.JSON.stringify(this);};GR34.prototype._addListener=function(_l,_lArr){if(!_l.id)
...
this.chat.setAgentConfig(aid,buID,agID,agentAttrs);if(!noSave){this.save();}
...
+eventData+','+buID+','+agID+')';win.opener.inqFrame.setTimeout(delayedCommand,50);}}catch(er){log("FAILED ATTEMPT TO SET AGENT ID FROM FLASH: "+er);}};GR34.prototype.isV3Continue=function(){var ciData=this.getChatInterfaceData();return ciData?ciData.c==1:false;};GR34.prototype.isV3Active=function(){if(CM.xd){try{var activeFlag=CM.cookies["inqCA"];var activeCount=0,e=null;if(activeFlag==null)
return false;if(isNaN(activeCount=parseInt(activeFlag))){return false;}
if(activeCount<=0)return false;return true;}catch(e){log("Error detecting isV3Active:"+e);return false;}}else{var ciData=this.getChatInterfaceData();if(!!ciData)
...
if(agID&&parseInt(agID)>0){custEvt.absorb({agentGroupID:agID});evt.absorb({agentGroupID:agID});}
if(buID&&parseInt(buID)>0){custEvt.absorb({businessUnitID:buID});evt.absorb({businessUnitID:buID});}
this._fireEvt(function(l,evt){if(typeof l.onChatEngagedEvent=="function")l.onChatEngagedEvent(evt);if(typeof l.onChatEvent=="function")l.onChatEvent(evt);},evt);};GR34.prototype.firePersistentChatClosed=function(_src){this.fireChatClosed(undefined,_src);};GR34.prototype.firePersistentChatLanding=function(){this._firePersistentLandingEvt(this.getChatData());};GR34.prototype.firePersistentPush=function(){this._fireChatEvt({id:++this.evtIdx,chatType:this.chat.getChatType(),evtType:this.EVTS.PERSISTENT_PUSH,chat:this.getChatData(),timestamp:new Date()});};GR34.prototype.firePendingEvents=function(){if(this.isPersistentChat()||this.isV3Continue()){this.firePersistentChatLanding();}};GR34.prototype._firePersistentPushEvt=function(evt){var chatData=null;var inh=null;if(this.chat){inh=this.chat.inHOP;chatData=this.chat.getChatData();}
evt=MI8.mixAbsorber({id:this.evtIdx++,evtType:this.EVTS.PERSISTENT_PUSH,src:this,chatID:(chatData?chatData.id:null),inHOP:inh});evt.absorb(chatData).absorb({"customerID":Inq.getCustID(),rule:this.rule});this._fireEvt(function(l,_evt){if(l.onPersistentPush)l.onPersistentPush(_evt);},evt);};GR34.prototype._firePersistentLandingEvt=function(evt){this._fireEvt(function(l,_evt){if(l.onPersistentChatLanding)l.onPersistentChatLanding(_evt);},evt);};GR34.prototype.fireAgentAssigned=function(_agtID,_cobrowseEnabled,eventData,buID,agID){try{this.evtIdx++;var evt=MI8.mixAbsorber({id:this.evtIdx,agentID:_agtID,cobrowseEnabled:!!_cobrowseEnabled,ruleID:this.chat.getRule().getID(),chatID:this.getChatID(),timestamp:new Date(),businessUnitID:buID});if(agID){evt.agentGroupID=agID;}
...
this._fireEvt(function(l,evt){if(l&&typeof l.onAgentAssigned=="function"){l.onAgentAssigned(evt);}},evt);}catch(e){logMessageToTagServer("Failure to fireAgentAssigned: "+catchFormatter(e),LOG_LEVELS.INFO);}};GR34.prototype.isServiceSupported=function(chatType){return!!this.CHAT_TYPES[chatType];};GR34.prototype.isCallServiceType=function(chatType){return(this.CHAT_TYPES.C2CALL==chatType||this.CHAT_TYPES.POPUP_CALL==chatType);};GR34.prototype.getConversionType=function(){if(!!this.chat){return this.isCallServiceType(this.getChatType())?"call":"chat";}else{return null;}};GR34.prototype.isServiceInProgress=function(chatType){var inProgress=false;if(!!this.chat){var thisChatType=this.chat.getChatType();if(chatType=="ANY"){return!isNullOrUndefined(thisChatType);}
...
if(!c2c.isClicked()){this.fireC2CClicked(c2c);var self=this;CM.updateCACookie(function(){CHM.request(c2c.getRule(),c2c.getChatType(),c2c.getXmlChatSpec(),callThisNumber||null,c2c.c2p,function(){self.showDisabledIcon();});});c2c.setClicked(true);X43.lastusedC2CId=c2c.pageElementID;}
...
this._fireEvt(f,evt);};X43.prototype.IMAGETYPES={disabled:"d",busy:"b",afterHours:"ah",ready:"r"};X43.prototype.STATES={"d":"chatactive","b":"busy","ah":"outofhours","r":"ready"};function Chat(chatData){this.sesVar=new VBER34("chat",{},resources["session"]);this._className="Chat";this._stage=window.parent.document.getElementById("inqChatStage");this.intervalId=0;this._isVisible=false;this.dragHandleID="inqTitleBar";this.cd=MI8.clonize(chatData);this.rule=null;this.agID=null;this.BEACON_DATA="DATA";}
...
var agID=this.getAgentGroupID();if(!isNullOrUndefined(agID)){flashvars+='&agID='+agID;}
...
return result;};Chat.prototype.getAgentID=function(){return isNullOrUndefined(this.cd.aid)?"":this.cd.aid;};Chat.prototype.setAgentConfig=function(aid,buID,agID,agentAttrs){this.cd.aid=isNullOrUndefined(aid)?"":aid;this.cd.buID=buID;this.cd.agID=agID;this.cd.agtAttrs=this.agentAttributesStringToMap(agentAttrs);CHM.getLastChat().businessUnitID=buID;CHM.getLastChat().agentGroupID=agID;CHM.getLastChat().agentID=aid;};Chat.prototype.agentAttributesStringToMap=function(attrs){function split2(str,sep){var index=str.indexOf(sep);if(index<0){return[str,""];}else{return[str.substring(0,index),str.substring(index+sep.length)];}}
...
return false;};Chat.prototype.getChatBusinessUnitID=function(){return isNullOrUndefined(this.cd.buID)?-1:this.cd.buID;};Chat.prototype.getChatAgentGroupID=function(){return isNullOrUndefined(this.cd.agID)?null:this.cd.agID;};Chat.prototype.getScriptID=function(){var id=0;try{id=this.chatSpec.stId;}catch(e){}
...
return 50;}
function getPopupCloserWidth(){try{return Inq.CHM.chat.chatSpec.chatTheme.ciw;}catch(e){}
return 50;}
...
function isCobrowseEngaged(){try{return getCBC().isCobrowseEngaged();}catch(e){logError("CBC ERROR: ",e);}
return false;}
function isSharedControl(){try{return getCBC().isSharedControl();}catch(e){logError("CBC ERROR: ",e);}
return false;}
...
return true;}
function createXFrame(div,url,businessUnitID,scrolling,data,initiator,location){var ldr=new XFormsLoader();return ldr.createXFrame(div,url,businessUnitID,scrolling,data,null,initiator,location);}
...
return ROM.composeURL(data.altURL,{chatID:CHM.getChatID(),agentID:CHM.getAgentID(),clientID:siteID,BRName:brName,custID:Inq.getCustID(),businessUnitID:data.businessUnitID,sessionID:data.sessionID,stage:data.stage,incAssignmentID:data.incAssignmentID,siteID:data.siteID,surveyContent:data.surveyContent});}
...
else{log("Popout Window Error: winName is required but not provided.");}}else{log("Popout Window Error: Web resource not found for popout window for resource ID = "+webResourceID);}};this.getKeySurveyIdFromSurveySpecAltUrl=function(surveySpecID,overrideURL){var url=overrideURL;var spec=this.getSurveySpec(surveySpecID);if(!url&&spec&&spec.altURL){url=spec.altURL;}
if(url){var found=url.substring(url.indexOf("/f/")+3,url.lastIndexOf("/")).split("/");if(found&&found.length){return found[0];}}
return surveySpec.id;};this.showSurvey=function(surveySpecID,rule,brSurveySpecOverride,preSurveyDataPass,windowFeatures,surveyUrlOverride,surveyReporting){var windowSpec=windowFeatures?(","+windowFeatures+","):', statusbar=0, menubar=0, scrollbars=1, resizable=1, toolbar=0, copyhistory=0,';var data=surveyDataMap[surveySpecID];if(surveyUrlOverride){windowURL=surveyUrlOverride;}
if(data){MI8.mixAbsorber(data);if(brSurveySpecOverride){data.absorb(brSurveySpecOverride);}
if(surveyUrlOverride){surveyURL=surveyUrlOverride;}
try{var lastChat=CHM.getLastChat();if(surveyReporting){var surveyID=this.getKeySurveyIdFromSurveySpecAltUrl(surveySpecID);var automatonStartedData={_domain:"automaton",evt:"started",automatonID:"SAT_"+surveyID,automatonStartedBy:("BR,"+(CHM.getChat()?CHM.getChat().getRuleId():(evt.rule?evt.rule.id:rule.getID()).toString())),startedIn:"div",automatonType:"satisfactionSurvey",businessUnitID:CHM.getBusinessUnitID(),chatID:lastChat.id,customerID:Inq.getCustID(),incAssignmentID:getIncAssignmentID(),sessionID:getSessionID(),siteID:getSiteID(),pageID:LDM.getPageID(),businessRuleID:lastChat.brID,deviceType:getDeviceType(),operatingSystemType:getOSType(),browserType:getClientBrowserType(),browserVersion:getClientBrowserVersion(),agentID:lastChat.agentID?lastChat.agentID:""};ROM.send(urls.logDataURL,automatonStartedData);var formDataAddition={chatID:lastChat.id,customerID:Inq.getCustID(),incAssignmentID:getIncAssignmentID(),sessionID:getSessionID(),visitorAttributes:VAM.getCopyAsArray(false).join(";"),automatonAttributes:"",siteID:getSiteID(),pageID:LDM.getPageID(),busUnitID:CHM.getBusinessUnitID(),businessRuleID:lastChat.brID};surveyUrlOverride=data.altURL;if(preSurveyDataPass){MI8.mixAbsorber(preSurveyDataPass).absorb(formDataAddition);}
...
if(preSurveyDataPass){if(!surveyUrlOverride){log("Can't submit survey with POST because URL is not provided");return;}
...
try{this.surveyWindow=window.parent.open(prepareURLForPostRequest(surveyUrlOverride),"",windowFeatures?windowFeatures:("left="+x+",top="+y+",width="+w+",height="+h+",scrollbars=yes,resizable=yes"));}catch(e){log("Can't launch survey window :"+e);}
if(typeof this.surveyWindow!=="undefined"&&this.surveyWindow!=null){var xLoc=this.surveyWindow.screenX!=undefined?this.surveyWindow.screenX:this.surveyWindow.screenLeft;this.surveyWindow.moveTo(xLoc,20);if(preSurveyDataPass){surveyURL=prepareURLForPostRequest(surveyUrlOverride);var survForm=this.surveyWindow.document.createElement("form");survForm.name="surveyForm";survForm.id="inqSurveyForm";survForm.action=surveyUrlOverride;survForm.method="POST";survForm.style.display="none";for(var k in preSurveyDataPass){if(typeof preSurveyDataPass[k]==="function"||typeof preSurveyDataPass[k]==="object"||k=="height"||k=="width"){continue;}
...
log("Survey URL: "+surveyURL);}}else{var windowURL=(data.altURL.length>0)?getAltSurveyURL(data,rule):ROM.composeURL(surveyURL,data)
...
C2CM.reset();LDM.init(true);BRM.reset();BRM.resetForReinit();BRM.init(true);BRM.start();LDM.start();}else{if(pageID&&pageID>0){win.inqSiteID=pageID;}
...
html=html.replace('</head>','<script type="text/javascript">if (typeof DOCUMENT_REWRITE_BUG != "undefined") {top.inqFrame.Inq.FlashPeer.formsLoader.resetGlobalVars(window);}</script></head>');if(window.location.host!=document.domain)
...
return jsidVar.getValue();};XFormsLoader.persistJsid=function(jsid){PM.getVar(XFormsLoader.JSESSIONID).setValue(jsid);};return XFormsLoader;}();function CBMR(id,config,cobrowseBannerText){this.cobrowseBannerText=cobrowseBannerText;this._frameworkModule(id);this._config=config;}
...
return values;};this.getCopyAsMap=function(){var copy={currentVisitorAttributes:this.currentVisitorAttributes,keys:function(){var keys=[];for(var a in this.currentVisitorAttributes){keys[keys.length]=a;}
