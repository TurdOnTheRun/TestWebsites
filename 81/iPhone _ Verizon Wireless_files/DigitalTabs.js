(function() {
   ...
   */
  function DigitalTabs(element) {
    this._element = element;
    this._tablist = this._element.querySelector('.' + this._CssClasses.TABLIST);
    this._tabs = this._tablist.querySelectorAll('.' + this._CssClasses.TAB);
    this._tabpanels = this._element.querySelectorAll('.' + this._CssClasses.TABPANEL);
    this._init();
  }
   ...
   * @private
   */
  DigitalTabs.prototype._CssClasses = {
      TABS: 'digi-tabs',
      TABLIST: 'digi-tabs__tablist',
      TAB: 'digi-tabs__tab',
      TABPANEL: 'digi-tabs__tabpanel',
      IS_ACTIVE: 'is-active',
      IS_HIDDEN: 'is-hidden',
   ...
   * @private
   */
  DigitalTabs.prototype._KeyCodes = {
      ENTER: 13,
      LEFT: 37,
      UP: 38,
      RIGHT: 39,
  ...
  /**
   * Initializes Digital tabs by initializing
   * tabs and tabpanels
   *
   * @private
   */
  DigitalTabs.prototype._init = function() {
    this._initTabs();
    this._initTabpanels();
  };

  /**
   * Initializes Digital tab by initializing tabs
   * set initial aria attributes and create
   * new Digital tab for each element
   *
   * @private
   */
  DigitalTabs.prototype._initTabs = function() {
    for (var i = 0; i < this._tabs.length; i++) {
      if (i !== 0) {
        this._tabs[i].setAttribute('aria-selected', 'false');
        this._tabs[i].setAttribute('tabindex', '-1');
      } else {
        this._tabs[i].setAttribute('aria-selected', 'true');
   ...
   * @private
   */
  DigitalTabs.prototype._initTabpanels = function() {
   ...
   * @private
   */
  DigitalTabs.prototype._resetTabState = function() {
    for (var i = 0; i < this._tabs.length; i++) {
      this._tabs[i].setAttribute('aria-selected', 'false');
   ...
   * @private
   */
  DigitalTabs.prototype._resetTabpanelState = function() {
    for (var i = 0; i < this._tabpanels.length; i++) {
      this._tabpanels[i].setAttribute('aria-hidden', 'true');
      this._tabpanels[i].classList.add(this._CssClasses.IS_HIDDEN);
    }
   ...
   * @param {DigitalTabs} ctx The DigitalTabs object that owns the tab.
   */
  ...
  componentHandler.register({
    constructor: DigitalTabs,
    classAsString: 'DigitalTabs',
    cssClass: 'digi-tabs'
  });
