/* Timestamp: Wed Apr 06 15:50:44 GMT-08:00 2016*/if(navigator.userAgent.indexOf("MSIE 6")>=0
     || (navigator.userAgent.indexOf("MSIE 7")>=0 && navigator.userAgent.indexOf("Trident")==-1 &&  navigator.userAgent.toLowerCase().indexOf("media center")==-1)){/* We do not support IE 6.X or 7.X any more */}
else if (window["v3Lander"]==null){

v3Lander={

    codeVersion:'1459986644898',
	v3Frame:false,
	...
	resizeIframe: function(width, height){
        /**
         *  @see http://jira.touchcommerce.com/browse/RTDEV-11045
         * */
	...
	prepareBeforeUnload: function(atxt, ctxt, cancelFcn, showImage, imageElement){
		window.onbeforeunload = function(evt){
            ...
            if(inqFrame.Inq.EC.update()){
                ...
                if(atxt && navigator.userAgent.indexOf("Firefox") < 0) {
                    alert(atxt);
                }
	...
	_reload: function(){
        var chatStage = document.getElementById("inqChatStage");
        var corner = document.getElementById("inqDivResizeCorner");
        var title = document.getElementById("inqTitleBar");
        if (chatStage) {
            document.body.removeChild(chatStage);
        }
        if (corner) {
            document.body.removeChild(corner);
        }
        if (title) {
	...
	createV3Frame: function(){
		...
		div.style.cssText = "z-index:9999999;overflow:hidden;position:absolute;height:1px;width:1px;left:0px;top:0px;"
						+"border-style: none;border-width: 0px;" ;
		div.innerHTML = '<ifr'+'ame id="inqChatStage" title="Chat Window" name="10004593" src="'+iframesrc+'"'
					  + ' style="z-index:9999999;overflow:hidden;position:absolute;height:1px;width:1px;left:0px;top:0px;border-style: none;border-width: 0px;display: none;"'
					  + ' scrolling="NO"'
					  + ' frameborder=0'
					  + '></iframe>'
		...
		if (iframesrc==this.hostedFileURL) {
			...
			doc.write(
				...
				+'\t\tsrc="' + v3Lander.normalizeProtocol('https://verizon.inq.com')+'/chatskins/launch/inqChatLaunch10004593.js"><\/script>\n'
				+'</body>\n'
				+'</html>');
			doc.close();
		}
    ...
    loadScript:function(name) {
        ...
        if (bdy && bdy[0]) {
            ...
            srctag.src = "https://verizon.inq.com".replace(/^HTTPS?:/i , window.location.protocol) + name;
    ...
    inqSiteDataFun:null,
    inqRulesEngineFun:null,
    mbusToLoad:[],
    page:null,
    cgIDs:null,
    isBupMode:false,
    isRplMode:false,
    loadedMbuRuleDataFuns:[],
    ...
    getContentGroupIDs: function(){
        return this.cgIDs;
    },

    
    resolvePage: function() {
        var requestUrl = "/tagserver/launch/resolvePage?siteID=10004593&url=" + encodeURIComponent(document.URL) + "&codeVersion=" + encodeURIComponent(v3Lander.codeVersion);
        var overriddenPageId = null;
        if (window.inqSiteID) {
            overriddenPageId = window.inqSiteID;
        } else if (window.parent && window.parent.inqSiteID) {
            overriddenPageId = window.parent.inqSiteID;
        }

        if (overriddenPageId) {
            requestUrl += "&overridePageId=" + encodeURIComponent(overriddenPageId);
        }
        this.loadScript(requestUrl);
    },

    
    applyPage: function(data) {
        ...
        var prepareMbusList = function (bUnits) {
            var mbusToLoad = [];
            // check duplicate and push
            for (var i = 0; i < bUnits.length; i++) {
                if (!contains(mbusToLoad, bUnits[i])) {
                    mbusToLoad.push(bUnits[i]);
                }
            }
            return mbusToLoad;
        };

        if (this.isBupMode && data.bUnits) {
            this.mbusToLoad = prepareMbusList(data.bUnits);
        }

        if (this.isRplMode && data.page) {
            this.page = data.page;
        }

        if (this.isRplMode && data.cgIDs) {
            this.cgIDs = data.cgIDs;
        }
    ...
    loadTcFramework:function() {
        v3Lander.loadScript("/chatskins/launch/tcFramework.min.js?codeVersion=" + encodeURIComponent(v3Lander.codeVersion));
    },

    loadMbuData:function() {
        for (var i = 0; i < parent.v3Lander.mbusToLoad.length; i++) {
			this.loadMbuDataForBU(parent.v3Lander.mbusToLoad[i]);
        }
    },

	buDataCallbackFcn: null,

	/***
	 * Calls to tagserver to load the appropriate rules file for a given business unit.
	 * @param buID {Number}
	 * @param callback [Function]
	 */
	loadMbuDataForBU:function(buID, callback){
		if(!!callback){
			this.buDataCallbackFcn = callback;
		}
		v3Lander.loadScript("/chatskins/launch/site_10004593_bu_" + buID + ".js?codeVersion=" + encodeURIComponent(v3Lander.codeVersion));
	},

    applyMbuRuleData:function(mbuRuleData) {
		if(this.buDataCallbackFcn){
			this.buDataCallbackFcn(mbuRuleData);
			this.buDataCallbackFcn = null;
		}
		else{
			v3Lander.loadedMbuRuleDataFuns[v3Lander.loadedMbuRuleDataFuns.length] = mbuRuleData.toString();
			// check that we already loaded exactly the same amount of mbu rule data and initiate framework loading
			if (v3Lander.loadedMbuRuleDataFuns.length == parent.v3Lander.mbusToLoad.length) {
				v3Lander.loadTcFramework();
			}
		}
    },
	...
	insertLandingTag:function(){
        ...
        v3Lander.loadScript("/chatskins/launch/site_10004593_default.js?codeVersion=" + encodeURIComponent(this.codeVersion));
     },
    
    insertPostToServerTag:function(){
        insertScript("/tagserver/postToServer.js?codeVersion=" + encodeURIComponent(this.codeVersion));

        function insertScript(name) {
            var srctag=document.createElement("SC"+"RIPT");
            srctag.src="https://verizon.inq.com" + name;
            srctag.type="text/javascript";
            srctag.charset="utf-8";
            srctag.language="javascript";
	...
	onDomReady:function(){
        ...
        if(!this.v3Frame) {
			if(this.isBupMode || this.isRplMode) {
				this.requestPage();
			}
			else {
				this.createV3Frame();
	...
	testReady:function(){
        ...
        if (typeof jQuery != "undefined" && jQuery.isReady) {
		...
		} else if (!v3Lander.v3Frame && false) {
			setTimeout("v3Lander.onDomReady()", 1);
            ...
            var newIeVersion = false;
            if (tridentVersion != null && tridentVersion.length == 2 && parseInt(tridentVersion[1]) >= 6) {
                newIeVersion = true;
            }
			if (document.readyState=="complete" && newIeVersion) {
				setTimeout("v3Lander.onDomReady();", 0);
            } else if (!newIeVersion && document.all && !window.opera){       
				try {
					...
					if (isExitChat) {
					    ...
					    if (this.isAtHostedFileURL) {
							document.write('<iframe id="tcBBFrame" src="' + v3Lander.normalizeProtocol('https://verizon.inq.com')+'/tagserver/v3/blank.html" ' +
			...
			else if (document.addEventListener || (newIeVersion && document.attachEvent)){
    ...
    prepBBDetect: function(){
        ...
        var blankURL = v3Lander.normalizeProtocol("https://verizon.inq.com")+"/tagserver/v3/blank.html";
    ...
    else if (window.location.href.indexOf("?CCFRM") != -1) {
        var host = "https://verizon.inq.com".replace(/^https?:\/\//, ""); // Strip the protocol. Domain only.
        var url = "http://" + window.document.domain + "/tagserver/ccform/getCreditCardForm?host=" + host;
        window.location.replace(url);
    }
	else if (window.location.href.indexOf("?CCTKN=1") != -1) {
		var s = document.createElement("script");
		// Need to pass the parameters along...
		var params = window.location.href.replace(/^.*\?/,"?");
		var src = "https://verizon.inq.com/tagserver/cctoken/getToken" + params;
		s.src = src;
		document.head.appendChild(s);
	}
	else if (window.location.href.indexOf("?CCTKN=2") != -1) {
		var s = document.createElement("script");
		// Need to pass the parameters along...
		var params = window.location.href.replace(/^.*\?/,"?");
		var src = "https://verizon.inq.com/tagserver/cctokentest/getToken" + params;
		s.src = src;
		document.head.appendChild(s);
	}
    ...
    else if (window.location.href.indexOf("?POST2SERVER")!=-1) {
...
/* Timestamp: Wed Apr 06 15:50:44 GMT-08:00 2016*/
