var V2WidgetMediator = function(rootHTMLElement) {
	var $ = jQuery;
	...
	if(hasCarousel){
		...
		new RSThumbScrollBehavior(slider, {videoScope:videoScope, pageSize:3});
	  ...
	  $('.lp-widget.v2 .video-small-thumbs').css('position', 'absolute');
	  $('.lp-widget.v2 .video-small-thumbs').css('z-index', '-100000');
	  $('.m2 .video-small-thumbs').css('z-index', '0');
	
		$thumbs.mouseover(function(e) {
			$(this).find('.video-small-thumbs').stop().animate({"height":"98px","width":"159px","left":"-2px","top":"-1px"}, 100,'swing');
		}).mouseout(function(){
			$(this).find('.video-small-thumbs').stop().animate({"height":"96px","width":"156px","left":"0px","top":"0px"}, 100,'swing');
		});

	}

	function onAdStart() {
		console.debug("***** V2 onAdStart()");
		disableThumbs();
		$pdk.controller.disablePlayerControls(true, null, [videoScope]);
	}

	function onAdEnd() {
		console.debug("***** V2 onAdEnd()");
		enableThumbs();
	...
	function setSelectedVideo(videoInfo) {
		...
		updateDetails();
		setEmbedButtonClickHandler(videoInfo);
	...
	function setEmbedButtonClickHandler(videoInfo) {
		...
		var embedBtn = $("#v2-social-shares-"+myWidgetID+" .cs-embed .cs-share");
	...
	function setUpNextBanner() {
		if(slider){
			...
			console.debug("nextThumb : ", $nextThumb);
	...
	function updateDetails() {
		...
		$myDetailDiv.find('.video-title').dotdotdot({ellipsis:'...',height:100});
		$myDetailDiv.find('.video-description').dotdotdot({ellipsis:'...',height:48});
		setSocial();
	}

	function setSocial() {
		var socialBarConfig = {
			"containerID": "v2-social-shares-"+myWidgetID,
	...
	$pdk.ready(function() {
		if (console && console.debug) console.debug("****V2 PDK READY****");
		...
		*/
		...
		$pdk.controller.addEventListener("OnReleaseEnd", function(playlist) {
			...
			if(!evideo.playersLoaded && !evideo.isiPad){
				return;
			}

			if (!skipReleaseEndEvent) {
				if(evideo.isiPad) {
					// compensates for video load delay in iPad
					// otherwise player skips two ahead
					$("#"+myWidgetID+" .player div:nth-child(5)").hide();
					setTimeout(function() { 
						onVideoEnded(selectedVideoInfo); 
						$("#"+myWidgetID+" .player div:nth-child(5)").show();
					}, 1000);
				} else {
					onVideoEnded(selectedVideoInfo);
				}
