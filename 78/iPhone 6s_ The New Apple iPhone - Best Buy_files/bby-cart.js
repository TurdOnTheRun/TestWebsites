/* UI BUILD: Friday, Apr  1 2016 at 10:13:34 AM -- BUILD ID: BRANCH_NAME: com.bestbuy.atg-apps.release.1615 VERSION: 16.15.113 */
/* MD5: 90c06b8b1dfb108d86c0dc03ffd9bb68 */
...
bby.cart.controllers.salesFlow.prototype = {
    ...
    addToCart: function(caller, json){
        require(['_addToCart/javascript/addToCart'], function(addToCart) {
          var product = $.dto.parseJson(json).toObject();
          if(product.lpnId) {
            product.licensePlate = product.lpnId;
            product.dnm = true;
          }
          product.skuId = product.skuId.toString();
          product.productId = product.productId.toString();

            var options = _.pick(product, ['skuId', 'productId', 'sellerId', 'listingId', 'storeId', 'licensePlate', 'dnm']);
            if (options.sellerId && (!options.listingId || options.listingId === "")) {
                delete options.sellerId
            }
            
            addToCart(options).then(function(data) {
            if(data.isICR && templateName && templateName !== 'STF') {
              // Handle ICR, but not on Store Finder
              ...
              require(['sc-icr-lightbox/javascript/icrLightBox'], function(icrLightBox) {
                icrLightBox({cid: data.commerceItemId, skuId: data.skuId});
              });
            } else if(data.isHACCS) {
              require(['_accessoryFinder/javascript/_accessoryFinder'], function(AccessoryFinder) {
                AccessoryFinder({cid: data.commerceItemId, skus: data.skuId, onError: 'redirectToCart'});
              });
            } else {
              window.location = "/site/olspage.jsp?id=pcat17005&type=page";
            }
          }, function(data) {
            var $errorMsg = $('<div class="css-displayMessage bdt-displayMessage css-error"><img class="css-image" src="http://img.bbystatic.com/BestBuy_US/store/styles/images/transparent.gif"> <span class="css-text">' + data.msg + '</span></div>')
            // Handle errors on DNM list pages
            if($('#listView.clearance').length > 0) {
              $('#listView.clearance > .css-displayMessage').remove();
              $('#listView.clearance').prepend($errorMsg);
            }
            // Handle errors on Store Finder and DNM PDP
            if($('.css-displayMessage.bdt-displayMessage').length > 0) {
              $('.css-displayMessage.bdt-displayMessage ~ .css-error').remove();
              $('.css-displayMessage.bdt-displayMessage').after($errorMsg);
              if($('.checkout-delivery1.store-finder').length > 0){
