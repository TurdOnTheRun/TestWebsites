...
(function ($) {
	com.nature.rjHomePage = {
		...
		brightcoveExperienceModule: null,
		/*
		 * com.nature.rjHomePage.brightcoveOnTemplateLoad
		 * Called by Brightcove when the template loads (onTemplateLoads event).
		 * Retrieves Brightcove's experience module so it can be used later on the 'onTemplateReady' event.
		 *
		 * @param experienceId
		 *	Brightcove's 'Experience ID' which always takes the same value as far as there's only one video on a page
		 */
		brightcoveOnTemplateLoad: function (experienceId) {
			var player = brightcove.api.getExperience(experienceId);
			this.brightcoveExperienceModule = player.getModule(brightcove.api.modules.APIModules.EXPERIENCE);
		},
		/*
		 * com.nature.rjHomePage.brightcoveOnTemplateReady
		 * Called by Brightcove when the template is ready (onTemplateReady event)
		 * Sets up an event listener so when the browser's orientation changes,
		 *  we use Brightcove's API to force a redraw of the video player with the new dimensions.
		 * This fixes COPS-1297 (Brightcove videos on home pages not being resized on orientation change on iPads)
		 *
		 * @param evt
		 *	Event, not actually used in the code
		 */
		brightcoveOnTemplateReady: function (evt) {
			window.addEventListener("orientationchange", function () {
				this.brightcoveExperienceModule.setSize($('#video-container').width(), $('#video-container').height());
			}, false);
		},
		trendingTabs: function (activeTab) {
			if (this.hasLoaded === false) {
				...
				var $contentDestination = $('#home-trending .holder');
		...
		initTabs: function () {
			...
			$('.pubserv-tab-group').each(function () {
				var $this = $(this),
				t = new com.nature.TabGroupPubserv($this);
				...
				// add the tabbed box css classes
				$this.find('.tab-content > div').addClass('box box-border-dark');

				// initialise tabs
				t.init({
					activeTab: 1,
					breakpoints: ['js-box-3']
				});
				...
				t.subscribe('tab_switch', self.pageManager.trackTabs, self.pageManager);
