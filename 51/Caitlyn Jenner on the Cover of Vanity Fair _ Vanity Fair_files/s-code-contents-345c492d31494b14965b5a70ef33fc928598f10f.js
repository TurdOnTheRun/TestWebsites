...
s.linkInternalFilters = 'vanityfair.com,ci.vanityfair.com,advancemags.com,javascript:';
s.server = "";
s.channel = "";
s.pageType = "";
s.currencyCode = "USD";
...
function s_doPlugins(s) {
    ...
    //Captures unique content count in c58/v58
    //Check if beginning of session && if localStorage.cId exists
    if(s.pdvalue == 1){
      if(localStorage.cId){
    ...
    var contentId = _satellite.getVar('Content ID');
    var contentType = _satellite.getVar('Content Type');
    var contentIdArr = []; //maybe??
    var contentCount = "";

    //get contentId
    //if contentId && contentType <> homepage, index, search then push id to array
    if(contentType !== "homepage" && contentType !== "index" && contentType !== "search" && contentType !== "subindex" && contentType !== "error" && contentId !== "null" && contentId !== undefined){
        
        if(!localStorage.getItem('cId')){
            contentIdArr = [];
            contentIdArr.push(contentId);
            contentId = contentIdArr.join('|');
        } else { //if localStorage.cId does not exist
            contentIdArr = localStorage.cId.split('|');
            contentIdArr.push(contentId);
            contentId = contentIdArr.join('|');
        }
        localStorage.setItem("cId", contentId);
        
    } 
    else {
        if(localStorage.cId){
            contentIdArr = localStorage.getItem('cId');
            contentIdArr = contentIdArr.split('|');
        }
    }

    var uniqueIdArr = contentIdArr.filter(function(elem, pos,arr) {
    return arr.indexOf(elem) == pos;
    });

    //counting unique values
    var counts = {};
    for (var i = 0; i < uniqueIdArr.length; i++) {
        counts[uniqueIdArr[i]] = 1 + (counts[uniqueIdArr[i]] || 0);
    }
    contentCount = i;
    s.prop58 = s.eVar58 = contentCount;

    /* New or repeat visitor designation */
    s.eVar23 = s.getNewRepeat();
    s.prop23 = 'D=v23'
    
    //Time parting plugin call - change it to a valid s variable
    ...
    //SC ID sync  
    var sc = condeDIL.helpers.getCookie("s_vi");
    if (sc) {
